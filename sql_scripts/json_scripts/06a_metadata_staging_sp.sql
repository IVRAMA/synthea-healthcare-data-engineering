...
DECLARE
  ...
  C_NEW_TABLES CURSOR FOR
    ...
    FROM NEW_TABLES T
    JOIN STAGING.SYNTHEA_FLATTENED_L2 L2
...
BEGIN
  ...
  WITH DIFF_RAW AS (
    ...
    FROM   STAGING.SYNTHEA_FLATTENED_L2
  ...
  FOR REC IN C_NEW_TABLES DO
    V_SQL := 'CREATE OR REPLACE TABLE "' || REC.TABLE_NAME || '" (' ||
  ...
  FOR REC2 IN C_NEW_COLS DO
    V_SQL := 'ALTER TABLE "' || REC2.TABLE_NAME || '"' ||
...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE OR REPLACE TEMP TABLE TMP_SYNTHEA_FLATTENED_L1 
                     AS SELECT * FROM STAGING.SYNTHEA_FLATTENED_L1 WHERE BATCH_NO = ''' || :p_batch_no || '''';
                     
  EXECUTE IMMEDIATE 'CREATE OR REPLACE TEMP TABLE TMP_SYNTHEA_FLATTENED_L2 
                     AS SELECT * FROM STAGING.SYNTHEA_FLATTENED_L2 WHERE BATCH_NO = ''' || :p_batch_no || '''';
