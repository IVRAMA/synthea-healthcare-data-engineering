#!/usr/bin/env python3
"""One-time JSON bronze load: 99 json files loaded to master json table → Snowflake."""
import subprocess
...
SQL_FILES = [
    ...
    "07_Create_snowflake_tasks.sql",
    "08_execute_snowflake_tasks.sql"
]

def main():
    script_dir = Path(r"C:\dbt_projects\healthcare_db\json_scripts")
    log_dir = Path("C:\dbt_projects\healthcare_db\json_logs") / f"load_{pd.Timestamp.now().strftime('%Y%m%d_%H%M')}"
