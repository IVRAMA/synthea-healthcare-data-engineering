...
select distinct
    ...
    immunization_code,
    procedure_code
from {{ ref('J_fact_encounters') }} e
left join {{ ref('dim_dates') }} dt
    on e."START_TIME"::date = dt.date_day
left join {{ ref('J_fact_observations') }} o
    on e.encounter_id = o.ENCOUTER_ID
left join {{ ref('J_fact_medications') }} m
    on e.encounter_id = m.ENCOUNTER_ID
left join {{ ref('J_fact_immunizations') }} i
    on e.encounter_id = i.ENCOUNTER_ID
left join {{ ref('J_fact_procedures') }} pr
